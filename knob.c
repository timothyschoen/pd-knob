...
static void knb_doset(t_knb *x, t_floatarg f){
    float old = x->x_pos;
    x->x_fval = f;
    if(x->x_min > x->x_max){
        if(f > x->x_min)
...
static void knb_set(t_knb *x, t_symbol *s, int ac, t_atom *av){
    s = NULL;
    if(!ac)
        x->x_setpos = x->x_pos;
    else if(ac == 1 && av->a_type == A_FLOAT){
        knb_doset(x, atom_getfloat(av));
        x->x_setpos = x->x_pos;
    }
    x->x_setval = x->x_fval;
}
